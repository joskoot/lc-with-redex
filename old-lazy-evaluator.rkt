...
(define (lookup var env) (force (hash-ref env var var)))

(define (make-fun vars body env)
 (if (null? vars) (ev-aux body env)
  (let ((var (car vars)) (vars (cdr vars)))
   (λ (x) (make-fun vars body (hash-set env var x))))))
...
(define (clear-env) (set! env (make-immutable-hash)))
(define (list-vars) (hash-keys env))
(define (list-env) (hash-map env list))
(define (extend-env var value) (set! env (hash-set env var value)))
